    ...
    def train(self, dataset: torch.utils.data.Dataset):
        ...
        for _ in progress_bar:
            ...
            for batch_idx, (batch_x, batch_y) in enumerate(train_loader):
                ...
                likelihood_loss = CrossEntropyLoss()
...
class BayesianLayer(nn.Module):
    ...
    def __init__(self, in_features: int, out_features: int, bias: bool = True):
        ...
        self.prior_mean = 0.0
        self.prior_sigma = 0.25
        self.pi = torch.tensor(0.5)
        
        self.sigma_1 = torch.tensor(2.0) # [1] != [] 
        self.sigma_2 = torch.tensor(0.5) # -> ()
        # self.mean = torch.nn.Parameter(torch.Tensor(out_features, in_features))
        ...
        # create a prior with N(0, sigma) --> using scale mixture here, so N(0, pi * sigma_1 + pi * sigma_2)
        self.prior = UnivariateGaussian(
            mu = torch.tensor(self.prior_mean),
            sigma = torch.tensor(self.prior_sigma) # (self.pi * self.sigma_1)**2 + ((1-self.pi) * self.sigma_2)**2
        )

        # Uncomment the following, to use the mixture component.
        # self.prior = MixtureGaussian(
        #     pi=torch.tensor(0.5),
        #     sigma_1 = torch.tensor(0.9),
        #     sigma_2 = torch.tensor(0.1)
        # )
        ...
        #      torch.nn.Parameter(torch.zeros((out_features, in_outfeatures))),
        #      torch.nn.Parameter(torch.ones((out_features, in_features)))
        #  )


        # mu and rho need to be vectors / matrices of the same shape [m, n]
        self.weights_var_posterior = MultivariateDiagonalGaussian(
              torch.nn.Parameter(torch.zeros((out_features, in_features)).uniform_(-0.2, 0.2)),
              torch.nn.Parameter(torch.ones((out_features, in_features)).uniform_(-5,-4))
        )
        ...
        if self.use_bias:
            ...
            self.bias_var_posterior = MultivariateDiagonalGaussian(
              torch.nn.Parameter(torch.zeros((out_features,1))), # TODO: leave out 1, maybe? (out_features)
              torch.nn.Parameter(torch.ones((out_features,1)))
        )
    ...
    def forward(self, inputs: torch.Tensor):
        ...
        # TODO: not sampling but log_likelihood calculation --> self.prior.log_likelihood(weoghts)
        # log_prior = self.prior.sample()
        #log_variational_posterior = self.weights_var_posterior.sample()
        
        
        if self.use_bias:
        	# bias = self.bias_var_posterior.sample() # add bias onto prior and var_pos
        ...
        return (inputs @ weights.T) + torch.sum(bias, axis=1), log_prior, log_variational_posterior
...
class BayesNet(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> typing.Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        for idx, current_layer in enumerate(self.layers):
            new_features, log_prior_new, log_variational_posterior_new = current_layer(current_features)

            # we need to add log[P(w)] and log[P(D|w)] over the layers
            log_prior += torch.sum(log_prior_new)
            log_variational_posterior += torch.sum(log_variational_posterior_new)

            if idx < len(self.layers) - 1:
                # if this is NOT the last layer, apply an activation function -> the last layer has a seperate one
                new_features = self.activation(new_features) # ReLU on F.linear layer
            
            current_features = new_features
        
        # normalize log_prior and log_variational_posterior with batch_size -> Monte-Carlo gradients?
        log_prior = log_prior / x.shape[0]
        log_variational_posterior = log_variational_posterior / x.shape[0]
...
class UnivariateGaussian(ParameterDistribution):
    ...
    def __init__(self, mu: torch.Tensor, sigma: torch.Tensor):
        ...
        assert sigma > 0
        self.mu = mu # these are already nn.Parameter --> we track them with Backprop
        self.sigma = sigma
        # TODO Yasin: check, if this is correct
        self.n:Normal = Normal(self.mu, self.sigma)

    def log_likelihood(self, values: torch.Tensor) -> torch.Tensor:
        # TODO: Implement this
        return self.n.log_prob(values)
...
class MultivariateDiagonalGaussian(ParameterDistribution):
    ...
    def log_likelihood(self, values: torch.Tensor) -> torch.Tensor:
        # TODO: Implement this
        # YTA: this should be correct, back-checked it with log_prob below
        return self.m.log_prob(values)
    ...
    def log_prob(self, input):
        ...
        # m = 1000 #values.size()
        # res = - m/2 * torch.log(torch.tensor(2*3.141529)) 
        # res += - m/2 * torch.log(torch.det(self.rho)) 
        # res += - 1/2 * torch.sum(
        #     (values - self.mu).T * self.sigma * (values - self.mu)
        # )
    ...
    def sample(self) -> torch.Tensor:
        ...
        epsilon = Normal(0,1).sample(self.mu.shape)
        return self.mu + self.sigma * epsilon


class MixtureGaussian(ParameterDistribution):
    ...
    def __init__(self, pi, sigma_1, sigma_2):
        ...
        self.pi = pi
        self.dist_1 = torch.distributions.Normal(0, self.sigma_1)
        self.dist_2 = torch.distributions.Normal(0, self.sigma_2)

    def log_likelihood(self, values: torch.Tensor) -> torch.Tensor:
        mixture_component_1 = torch.exp(self.dist_1.log_prob(values))
        mixture_component_2 = torch.exp(self.dist_2.log_prob(values))
        mixture = self.pi * mixture_component_1 + (1 - self.pi) * mixture_component_2
